...
% ---------------------------------------------------------------------------------------
    ...
    for iSat = 1:nUsedSat
        ...
        if any(strcmp(corrMode, 'clck'))
            corr(svPRN)    =   corr(svPRN) - timeCorr(svPRN);
        end
        if any(strcmp(corrMode, 'iono'))
            corr(svPRN)    =   corr(svPRN) + ionoCorr(svPRN);
        end
        if any(strcmp(corrMode, 'tropo'))
            corr(svPRN)    =   corr(svPRN) + tropoCorr(svPRN);
        end
        prCorr(iSat)  =   pr(svPRN) - corr(svPRN);
    end
